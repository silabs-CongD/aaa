...
def pre_build_cmake():
    ...
    file = open(os.path.join(os.environ.get('GITHUB_WORKSPACE'), 
                             "solution_list.txt"), "r")
    slcp_project_path_list = []
    for line in file:
        if line.find(".slcp") != -1:
            slcp_project_path = os.path.join(os.environ.get('GITHUB_WORKSPACE'), 
                                             line.strip())
    ...
    for slcp_file in slcp_project_path_list:
        ...
        pre_build_makefile_path = os.path.join(os.environ.get('GITHUB_WORKSPACE'), 
                                               "scripts/Makefile")
